...
async def move_random_monster():
    ...
    while True:
        ...
        if not wandering_monsters_enabled or not monsters:
            continue  # Пропускаем итерацию, если режим выключен
        
        # Получаем список всех монстров и их позиций
        monster_pos = random.choice(list(monsters.keys()))
        monster_name = monsters[monster_pos]
        x, y = monster_pos

        # Пробуем переместить монстра в случайном направлении
        ...
        for dx, dy, direction in directions:
            ...
            # Проверяем, что на новой позиции нет монстра
            if game_field[new_x][new_y] is None:
                # Перемещаем монстра
                game_field[new_x][new_y] = game_field[x][y]
                game_field[x][y] = None
                del monsters[(x, y)]
                monsters[(new_x, new_y)] = monster_name
                # Отправляем сообщение о перемещении всем клиентам
                await broadcast_message('movemonsters', monster=monster_name, direction=direction)
                ...
                break  # Выходим из цикла, так как успешно переместили монстра
        ...
        while not reader.at_eof():
            ...
            if cmd == "locale":
                ...
                try:
                    ...
                    await broadcast_message('sayall', username=username, msg=msg_to_broadcast)
                except ValueError:
                    await clients[username].put("Invalid arguments")
